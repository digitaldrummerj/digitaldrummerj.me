...
function sass() {
  return gulp
    .src(path.join(THEME.source, '/scss/app.scss')) // SCSS import paths in `app.scss`
...
function javascript() {
  return gulp
    .src(PATHS.javascript) // JS import paths in `config.yml`
    .pipe($.sourcemaps.init())
    ...
    .pipe(
      $.if(
        PRODUCTION,
        $.uglify() // In production, the file is minified
          .on('error', e => {
            console.log(e);
          })
      )
...
gulp.task('hugo-build-prod', code => {
  return cp
    .spawn('hugo', ['--config', PATHS.prodconfig, '-t', THEME.name, '-s', HUGO.root, ''], {
      stdio: 'inherit',
    })
    .on('error', error => log(colors.red(error.message)))
    .on('close', code);
...
gulp.task('hugo-server-prod', code => {
  return cp
    .spawn(
      'hugo',
      ['--config', PATHS.prodconfig, 'server', '-p', PORT, '-t', THEME.name, '-s', HUGO.root, ''],
      { stdio: 'inherit' }
    )
    .on('error', error => log(colors.red(error.message)))
    .on('close', code);
...
gulp.task('lint', function() {
  return gulp
    .src(path.join(HUGO.public, '/**/*.html'))
    .pipe(
      prettify({
        indent_size: 2,
