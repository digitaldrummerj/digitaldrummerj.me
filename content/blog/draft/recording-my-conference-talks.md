...
When I got started as a conference speaker in 2014, I would see [Jeremy Clark](http://www.jeremybytes.com/) recording his talks and I thought that is something that I should do as well.  First, it helps me improve as a speaker by seeing  how I actually was on stage versus how I think it went.  As well, it gives attendees the ability to watch a replay of the talk in case they missed something or weren't able to see it.  This is assuming though that the video is posted somewhere and advertised to the attendees, which I used to be really bad about.  It was difficult for a long time for me to watch myself giving a talk.

So in 2015, I started recording my talks.  Unfortunately, I didn't process most of those videos.  Now fast forward to 2017 and I made a decision at the start of the year that if I was going to take the time to record a talk I would process it and post it.  A side goal was to also see if I could post the video while at the conference so that I could ensure that I actually posted it.  For 2017, I posted 9 talks publicly and 15 talks as unlisted (paid workshops) to my [YouTube channel](https://www.youtube.com/channel/UCDMvOL1XSKclxwplUT0fzLA).  I managed to post all of the public talks while at the conference.

## General Recording Requirements

Before we get into the specifics, I have some basic recording requirements that guided me as I figured out this process.

1. First and foremost be able to record my conference talks as painlessly as possible with good audio and video quality
...
1. Be able to upload the videos while at the conference
1. Be able to reuse a majority of the equipment to record meetups that I attend but am not presenting at

{{< alert class="warning" >}}
Note: Many of the product links are Amazon affiliate links.
{{</alert>}}

## History

> If you don't care about the equipment that I tried out before settling on my current equipment, you skip this section and go to the Current Equipment section.
...
For my current setup, I have used it to record over 20 talks and it hasn't let me down yet.
...
1. Needs to record directly into the computer
...
* The range on the wireless is good
* It just worked when I plugged it in
* Since it is USB it records directly into the computer without an extra device

All of the video on my [YouTube channel](https://www.youtube.com/channel/UCDMvOL1XSKclxwplUT0fzLA) are using this microphone. Don't forget to subscribe to my channel to get notified when my talks are posted.
...
* The clip comes off of the microphone easily.  Never had it come off though when recording.
* The USB dongle is quite wide and tends to cover more than 1 port

To solve the battery problem, I purchased a set of [Rechargeable batteries](http://amzn.to/2EhYi6L).  I needed a charger since I didn't have one and AAA batteries as used for my [Logitech R400 Slide Advancer](http://amzn.to/2DQwUjD).
...
**Warning:** If you are using a room microphone, you most likely will hear a slight amount of reverb in the recording and you can't fix without getting an audio feed directly from the room mic.   Getting a direct feed for the room mic is not typically possible at most events.  To see an example of what I am talking about, check out my [Angular Mix Talk](https://www.youtube.com/watch?v=IV5i0znk9Os&t=12s).
{{</blockquote>}}

### Software

The two pieces of software that I needed were screen capture and video editing.  My main goal was to find something that was easy to use and had a very small learning curve.
...
1. Needed to be able to record my screen, the microphone and the option for system sounds
1. Needed to be to combine multiple sources into the final video (camera, screen, intro, outro, music, etc)
1. Needed to be able to show both the screen and presenter at the same time
1. Needed to be able to separate the audio from the video of a clip so I can get rid of the bad audio
...
1. Needed to be able to output in HD (1920x1080).  If you are doing any kind of showing of code, anything below 720p is typically to read in a video.
...
The camera is technically optional but since one of my goals is to use the videos to improve my speaking, I need a camera to record myself.
...
1. Can record at least 2 hours without running out of battery
1. Can be run off an external power battery or be plugged into a power outlet.  External battery is very important since there is not normally power available out in the audience at a conference.
1. Not related to conference talks, but I wanted to be able record my drums or concerts which meant I needed a good microphone built into the camera.  This was the deciding factor in the camera the I went with.

The finally requirement of being able to record drums or concerts was the deciding factor for the camera that I selected.  If I didn't have this requirement I would have went with a cheaper camera.
...
* It has audio gain so I do not have to worry much about it overloading the audio
* It has a flip around screen so I can see what is being recorded
* It is lightweight
...
* It advertises that it can be used as a 720p webcam but it looks horrible.  Looks more like a 240 resolution.  My [Logitech C922x](http://amzn.to/2GADeJD) is far superior even at 720p.
...
* About every 3.5 GB, you will get a new video file that you will have to splice together in editing

### Accessories

**Tripod:** I have to say that it was difficult to find a tripod that was full size, lightweight, and would still fit in my laptop backpack.  I went with the [Neewer Carbon Fiber Portable Mini Tripod](http://amzn.to/2DJQF83).  It extends to 56 inches and folds down to 13".
...
**External Battery for Camera:** Since the camera can run off 5V 1A, I can use the iPhone portable charger that I already had.  I am using the [UNU Superpak 10000 mAH](http://amzn.to/2EkgHzQ) and can get around 4 full phone charges out it.  It also has a really nice battery charge light so I know if I need to charge it or not.
...
**iPhone Wall Charger:** Since the camera does not come with the wall charger, I got a [cheap pack of 5V 1A wall chargers][http://amzn.to/2BHRr3O]
...
> Note: I just purchased a [camera backpack with a laptop compartment](http://amzn.to/2FyF8Jw) that I am going to try as a replacement for the case and my [existing OGIO Gambit Laptop Backpack](http://www.ogio.com/backpacks/gambit-laptop-backpack/spr4704908.html)
...
With this setup, we will have 2 audio tracks:  camera and screen capture.  We will need to sync the audio in editing and then remove the camera audio.  This sounds like a pain to do but I am going to give you the best trick that I ever learned that will enable you to sync the audio in 30 seconds.  The is the same reason why you see movies use the clapper at the start of a scene.
...
1. Either loudly snap your fingers or clap your hands (this makes everyone in the room stare at you) while standing in front of the camera
...
1. Roughly align the markers for the footage
...
1. Right-click on the camera footage that is in the timeline and select the separate audio and video.
1. Make sure that all of the camera audio is in the same row on the timeline and turn off the row in Camtasia.  I don't delete the audio as there are times where I want to bring in the room audio for a question from the audience.  If you remove the footage there is no easy way to get it back.

After a few times of doing this you will get really good at syncing multiple audio sources and even be able to sync the audio just by looking at it.
...
Now you are ready to record your talks.  Make sure to do some practice runs with the equipment to work out any kinks. Leave a comment below and let me know where your recordings are.
